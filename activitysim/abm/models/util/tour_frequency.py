...
from activitysim.core import config, workflow, asim_enum
...
def process_mandatory_tours(
    ...
    work_and_school_and_student = (
        tours_merged.mandatory_tour_frequency == asim_enum.MandatoryTourFrequency.work_and_school
    ...
    tours["destination"] = tours_merged.workplace_zone_id.where(
        (tours_merged.tour_type == asim_enum.TourPurpose.work), tours_merged.school_zone_id
    )
...
def create_joint_tours(
    ...
    # set these here to ensure consistency across different tour categories
    assert tour_category in asim_enum.TourCategory
    tours["tour_category"] = tour_category
